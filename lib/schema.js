...
Schema.prototype.build_layout = function(group) {
    ...
    var add_to_layout = function(name, resolved_type, type) {
        ...
        if(resolved_type.primitive) {
            ...
            var seq = resolved_type.seq;
            var resolved_seq = seq ? seq : false;
            layout.push({
                name: name.name,
                type: resolved_type.name,
                primitive: resolved_type.primitive,
                dynamic: type.dynamic,
                seq: type.seq || resolved_seq,
                opt: type.opt,
    ...
    for(var i = 0; i < group.fields.length; i++) {
        ...
        if(type.seq && resolved_type.seq) {
            throw new Error("Type could not be a double-seq (2-d array)");
        }
