...
Schema.prototype.resolve_type = function(type, list) {
    if(type.ref == false) {
        list.push(type.type);
        return;
    }
    ...
    if(symbol instanceof GroupDef) {
        var err = this.build_layout(this.symbols[type.type]);
        if(err) { return err; }

        list.push.apply(list, this.cache[type.type]);
    } else if(symbol instanceof TypeDef) {
        this.resolve_type(symbol.type, list);
    }
...
Schema.prototype.build_layout = function(group) {
    ...
    if(this.cache[name] != null) {
        return new Error("Recursive definition!");
    }
    ...
    for(var i = 0; i < group.fields.length; i++) {
        ...
        var err = this.resolve_type(field.type, layout);
        if(err) { return err; }
    }
...
Schema.prototype.build = function(cb) {
    this.cache = {};

    for(var i = 0; i < this.groups.length; i++) {
        var group = this.groups[i];
        if(this.cache[group.name.name] == null) {
            var err = this.build_layout(group);
